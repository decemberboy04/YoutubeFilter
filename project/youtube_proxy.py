...
# 导入敏感词审查模块
from sensitive_filter import check_and_filter_data, get_filter_stats, set_sensitivity_threshold, get_sensitivity_threshold
            ...
            if flow.response.text:
                try:
                    ...
                    filtered_response = {
                        "error": {
                            "code": 403,
                            "message": "Content filtered due to sensitive material",
                            "reason": "sensitiveContentFiltered"
                        },
                        "filtered_info": {
                            "original_url": flow.request.url,
                            "match_count": checked_response['sensitive_check']['total_matches'],
                            "threshold": threshold,
                            "sensitive_words": checked_response.get('sensitive_matches', [])
                        }
                    }
                    
                    flow.response.text = json.dumps(filtered_response, ensure_ascii=False)
                    flow.response.status_code = 403
                    flow.response.headers["Content-Type"] = "application/json"
                    flow.response.headers["X-Content-Filtered"] = "true"
                    
                except Exception as e:
                    ctx.log.error(f"响应过滤错误: {e}")
                    flow.response.text = "Content filtered due to sensitive material"
                    flow.response.status_code = 403
